  ...
  try {
    ...
    if (Project) {
      ...
      await sendEmail({
        ...
        subject: `Project Alloted !`,
        message: "You have been alotted with a project !",
      });
      res
        .status(200)
        .json({ message: "Congratulation , ProjectLead Is Created.", Project });
      return;
    }
  } catch (error) {
...
exports.GetAllProjectLead = async (req, res) => {
  try {
    ...
    if (Project) {
      res.status(200).json({
        message: "Congratulation , You Got The All Project ! ",
        Project,
      });
      return;
    }
...
exports.Updateproject = async (req, res) => {
  try {
    ...
    if (ProjectId) {
      const Project = await ProjectLead.findByIdAndUpdate(
        req.params.id,
        {
          User: User,
          ProjectName: ProjectName,
      ...
      if (Project) {
        res
          .status(200)
          .json({ message: "Congratulation Record is Updated", Project });
        return;
      }
    }
  } catch (error) {
...
exports.DeleteProductLead = async (req, res) => {
  try {
    ...
    if (!ProductId) {
      res.status(400).json({
        message: "OOPS !! Id Failled To Get For Delete The ProectLead ",
      });
    }
    ...
    if (ProductId) {
      ...
      if (!Product) {
        res
          .status(400)
          .json({ message: "Product Lead is Not find for productid" });
        return;
      }

      if (Product) {
        res.status(200).json({
          message: "Congratulation , Product lead is deleted ",
          Product,
        });
        return;
      }
