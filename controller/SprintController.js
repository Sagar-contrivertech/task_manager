...
exports.RegisterSprint = async (req, res) => {
  try {
    ...
    if (findprojectstatus.status === "success") {
      ...
      if (Sprints) {
        await sendEmail({
          ...
          message: "You Sprint has been Created !",
        });
    ...
    res
      .status(400)
      .json({ message: "Sprint register is not possible in catch" });
  }
};
...
exports.UpdateUseidSprint = async (req, res) => {
  try {
    ...
    if (!Sprints) {
      res
        .status(400)
        .json({ message: "Sprint With this user id is not in database" });
      return;
    }

    if (Sprints) {
      const SprintsUser = await Sprint.findByIdAndUpdate(
        req.params.id,
        {
          User: User,
          Project: Project,
      ...
      if (!SprintsUser) {
        res
          .status(200)
          .json({
            message: "Sprint with this user id is is not added in another try",
          });
        return;
      }
      ...
      if (SprintsUser) {
        res
          .status(200)
          .json({
            message: "Sprint with this user id is possible",
            SprintsUser,
          });
        return;
      }
  ...
  try {
    ...
    if (!Sprints) {
      res
        .status(400)
        .json({
          message: "Sprint With this user id is not for delete in database",
        });
      return;
    }
    ...
    if (Sprints) {
      ...
      if (!SprintsUser) {
        res
          ...
          .json({
            message:
              "Sprint with this user id is is not for delete in another try",
          });
        return;
      }
      ...
      if (SprintsUser) {
        res
          .status(200)
          .json({
            message: "Sprint with this user id is possible for delete",
            SprintsUser,
          });
        return;
      }
    ...
    res
      .status(400)
      .json({ message: "Sprint Delete is not possible for delete in catch" });
  }
};
